...
const DEFAULT_CONFIG = {
  center: [-84.30226, 33.87623], // [Longitude, Latitude] - Default: Atlanta
  radiusStepNm: 2,
...
export function Radar({ cfg }) {
  ...
  const aircraft = useMemo(() => {
    ...
    return Array.from(combined.values()).filter(a => {
      ...
      // Only show aircraft in highlightedTails
  ...
  useEffect(() => {
    ...
    map.getView().setCenter(fromLonLat([-84.30226, 33.87623]));

    const center = fromLonLat([-84.30226, 33.87623]);
    const rings = [];
  ...
  useEffect(() => {
    ...
    const center = fromLonLat([-84.30226, 33.87623]);
    const maxRadius = 4 * cfg.radiusStepNm * NM_TO_M;
  ...
  return (
    <main className="radar">
      ...
      <header className="hud">
        <div>
          <div className="eyebrow">YJFC AIR TRAFFIC</div>
        </div>
      </header>
